...
class Member:
    ...
    def __init__(self, nickname, room_name):
...
class MafiaServicer(mafia_pb2_grpc.MafiaServicer):
    ...
    async def JoinRoom(self, request, context):
        ...
        if len(cur.players) <= 3:
            ...
            self.rooms[request.room_name].member_profiles[request.username] = Member(request.username, request.room_name)
    ...
    async def GetGameStatus(self, request, context):
    ...
    async def VotePaper(self, request, context):
    ...
    async def ChangeUserInfo(self, request, context):
        if request.room_name not in self.room_names:
            return mafia_pb2.ChangeInfoResponse(flag=False, message='No such room')
        if request.username not in self.rooms[request.room_name].players:
            return mafia_pb2.ChangeInfoResponse(flag=False, message='No such player')
        if request.gender != 'None':
            self.rooms[request.room_name].member_profiles[request.username].gender = request.gender
        if request.email != 'None':
            self.rooms[request.room_name].member_profiles[request.username].email = request.email

        return mafia_pb2.ChangeUserInfoResponse(flag=True, message='Changes made')
    

    async def SetAvatar(self, request, context):
        if request.room_name not in self.room_names:
            return mafia_pb2.SetAvatarResponse(flag=False)
        if request.username not in self.rooms[request.room_name].players:
            return mafia_pb2.SetAvatarResponse(flag=False)
        
        prev_name = self.rooms[request.room_name].member_profiles[request.username].filename
        if request.filename != '':
            self.rooms[request.room_name].member_profiles[request.username].filename = request.filename

        return mafia_pb2.SetAvatarResponse(flag=True, old_filename=prev_name)
    

    async def RoomUsers(self, request, context):
        if request.room_name not in self.room_names:
            return mafia_pb2.RoomUsersResponse(users=[])
        return mafia_pb2.RoomUsersResponse(users=self.rooms[request.room_name].players)
